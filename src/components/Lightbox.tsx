    ...
    <AnimatePresence>
      {product && (
        <motion.div
          ...
          className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm px-4 sm:px-6"
          onClick={onClose}
        >
          <motion.div
            ...
            className="relative bg-card border border-border max-w-lg w-full p-5 sm:p-8 lg:p-12 max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close button */}
            <button
              onClick={onClose}
              className="absolute top-3 right-3 w-11 h-11 flex items-center justify-center text-cream/50 hover:text-gold transition-colors duration-700"
            >
